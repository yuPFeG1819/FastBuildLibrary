...
object ImageLoader {
     ...
     * */
    @JvmStatic
    fun loadImageToView(imageView: ImageView, config : ImageLoaderConfig.()->Unit){
        imageLoadHelper.loadImageToView(imageView,ImageLoaderConfig().apply(config))
    }
    ...
    fun loadBitmap(
        ...
        imageLoadHelper.loadBitmap(
            context,
            ImageLoaderConfig().apply(config),
            onBitmapReady,
            loadStartAction,
            loadErrorAction
        )
